...
O branch `main` deve priorizar o runtime atual, assets em uso, testes e documentação ativa.
...
Arquivos versionados que ainda são referenciados pelo `index.html` ou por documentação ativa devem permanecer no `main` até a migração correspondente.
